...
public interface TeacherApprovalRepository extends JpaRepository<TeacherApproval,Long> {
    @Query(value = "SELECT ta.id AS idTeacherApprovals, u.name AS userName, u.email AS userEmail, ta.status AS teacherApprovalsStatus , ta.approved_at " +
            "FROM teacher_approvals ta " +
            "JOIN users u ON ta.user_id = u.id " +
            "WHERE ta.status = :status", nativeQuery = true)
